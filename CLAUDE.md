...
PointWorks 是一个基于 Qt5、VTK 和 PCL 构建的三维点云处理应用程序（原名 CloudTool2）。提供点云可视化、滤波、地面/植被分割、变化检测（C2C/C2M/M3C2）、点云配准、曲面重建、分割等功能。支持多视窗显示、中英文国际化、嵌入式 Python 脚本引擎。
...
- **依赖隔离**: `pw_io` 对 `pw_ui_base` 为 PRIVATE 依赖；`libs/algorithm/` 零 VTK 依赖
...
- **进度管理**: 使用 `ProgressManager` 统一管理进度条，工具/插件通过 `m_progress->runAsync()` 执行异步任务
